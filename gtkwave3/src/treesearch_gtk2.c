...
widget = GLOBALS->mainwindow;
set_window_busy(widget);

for(i=fetchlow(GLOBALS->sst_sig_root_treesearch_gtk2_c_1)->t_which;i<=fetchhigh(GLOBALS->sst_sig_root_treesearch_gtk2_c_1)->t_which;i++)
        {
        struct symbol *s;
	if(i<0) break; /* GHW */
        s=GLOBALS->facs[i];
...
if(GLOBALS->sst_sig_root_treesearch_gtk2_c_1)
	{
	int fl = fetchlow(GLOBALS->sst_sig_root_treesearch_gtk2_c_1)->t_which;
	int fh = fetchhigh(GLOBALS->sst_sig_root_treesearch_gtk2_c_1)->t_which;
	int fz = fh - fl + 1;
	...
	if(fz > WV_RECURSE_IMPORT_WARN)
		{
		char recwarn[128];
		sprintf(recwarn, "Really import %d facilities?", fz);
