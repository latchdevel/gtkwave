...
GLOBALS->page_size_type_renderopt_c_1 = which;
...
if(gd_rc == GTK_RESPONSE_OK)
	...
	if(gtk_printer_accepts_ps(gp))
		...
		save_tmpfilename = tmpnam_2(NULL, &fd_dummy);
			...
			if(job_stat)
				{
				gtk_print_job_send(gpj, (GtkPrintJobCompleteFunc)wave_GtkPrintJobCompleteFunc,
						GLOBALS->gp_tfn,
						NULL);
...
if(GLOBALS->target_mutex_renderopt_c_1[0])
	{
	fileselbox("Print To PDF File",&GLOBALS->filesel_print_pdf_renderopt_c_1,G_CALLBACK(pdf_print_cleanup), G_CALLBACK(NULL), "*.pdf", 1);
	}
else
if(GLOBALS->target_mutex_renderopt_c_1[1])
	{
	fileselbox("Print To PS File",&GLOBALS->filesel_print_ps_renderopt_c_1,G_CALLBACK(ps_print_cleanup), G_CALLBACK(NULL), "*.ps", 1);
	}
else
if(GLOBALS->target_mutex_renderopt_c_1[2])
	{
	fileselbox("Print To MIF File",&GLOBALS->filesel_print_mif_renderopt_c_1,G_CALLBACK(mif_print_cleanup), G_CALLBACK(NULL), "*.fm", 1);
	}
else
	{
...
void renderbox(char *title)
{
	...
	if(GLOBALS->window_renderopt_c_6)
		{
		gdk_window_raise(gtk_widget_get_window(GLOBALS->window_renderopt_c_6));
		}
	return;
	}
...
GtkWidget *combo_box = gtk_combo_box_text_new ();
	...
	gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT(combo_box), render_targets[i]);
	}
	...
	gtk_box_pack_start (GTK_BOX (small_hbox), combo_box, TRUE, FALSE, 0);
        g_signal_connect (combo_box, "changed", G_CALLBACK (render_clicked), NULL);
    ...
    for(i=0;i<5;i++)
	{
	gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT(combo_box), page_size[i]);
	...
	gtk_widget_show (combo_box);
	gtk_box_pack_start (GTK_BOX (small_hbox), combo_box, TRUE, FALSE, 0);
        g_signal_connect (combo_box, "changed", G_CALLBACK (pagesize_clicked), NULL);
    ...
    for(i=0;i<2;i++)
	{
	gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT(combo_box), render_type[i]);
