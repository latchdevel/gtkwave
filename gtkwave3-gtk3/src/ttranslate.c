	...
	for(i=0;i<GLOBALS->num_ttrans_filters;i++)
		...
		if(GLOBALS->ttranssel_filter[i])
			...
			if(!strcmp(GLOBALS->ttranssel_filter[i], *GLOBALS->fileselbox_text))
				...
				if(GLOBALS->is_active_ttranslate_c_2) gdk_window_raise(gtk_widget_get_window(GLOBALS->window_ttranslate_c_5));
				return;
				}
			}
		}
...
if(GLOBALS->ttrans_filter[GLOBALS->num_ttrans_filters])
	...
	strcpy(GLOBALS->ttranssel_filter[GLOBALS->num_ttrans_filters], *GLOBALS->fileselbox_text);
        ...
        gtk_list_store_set (GTK_LIST_STORE(GLOBALS->sig_store_ttranslate), &iter,
                                    NAME_COLUMN, GLOBALS->ttranssel_filter[GLOBALS->num_ttrans_filters],
                                    -1);
	}
	else
	{
...
fileselbox("Select Transaction Filter Process",&GLOBALS->fcurr_ttranslate_c_1,G_CALLBACK(add_filter_callback_2), G_CALLBACK(NULL),"*", 0);
}
...
void ttrans_searchbox(char *title)
{
    ...
    XXX_gtk_table_attach (XXX_GTK_TABLE (table), frame2, 0, 1, 0, 253,
                        GTK_FILL | GTK_EXPAND,
                        GTK_FILL | GTK_EXPAND | GTK_SHRINK, 1, 1);

    GLOBALS->sig_store_ttranslate = gtk_list_store_new (N_COLUMNS, G_TYPE_STRING);
    GtkWidget *sig_view = gtk_tree_view_new_with_model (GTK_TREE_MODEL (GLOBALS->sig_store_ttranslate));
    ...
    gtk_tree_selection_set_select_function (GLOBALS->sig_selection_ttranslate, XXX_view_selection_func, NULL, NULL);
    ...
    for(i=0;i<GLOBALS->num_ttrans_filters;i++)
        ...
        gtk_list_store_append (GTK_LIST_STORE(GLOBALS->sig_store_ttranslate), &iter);
        gtk_list_store_set (GTK_LIST_STORE(GLOBALS->sig_store_ttranslate), &iter,
                                    NAME_COLUMN, GLOBALS->ttranssel_filter[i+1],
                                    -1);
        }
    ...
    XXX_gtk_table_attach (XXX_GTK_TABLE (table), frameh0, 0, 1, 253, 254,
                        GTK_FILL | GTK_EXPAND,
                        GTK_FILL | GTK_EXPAND | GTK_SHRINK, 1, 1);
