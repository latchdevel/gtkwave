...
gtk_editable_select_region (GTK_EDITABLE (widget),
                             0, gtk_entry_get_text_length(GTK_ENTRY(widget)));
...
void tracesearchbox(const char *title, GCallback func, gpointer data)
{
	...
	for(idx=0; idx<sizeof(item_mark_start_strings)/sizeof(struct item_mark_string); ++idx)
		...
		gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT(combo_box), item_mark_start_strings[idx].str);
		}
	...
	for(idx=0; idx<sizeof(item_mark_end_strings)/sizeof(struct item_mark_string); ++idx)
		...
		gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT(combo_box), item_mark_end_strings[idx].str);
		}
        ...
        gtk_box_pack_start (GTK_BOX (mark_count_hbox_end), combo_box, TRUE, FALSE, 0);
