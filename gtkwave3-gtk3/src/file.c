...
void fileselbox_old(char *title, char **filesel_path, GCallback ok_func, 
	GCallback notok_func, char *pattn, int is_writemode)
{
#if defined __MINGW32__
OPENFILENAME ofn;
...
gtk_tooltips_set_tip_2(label_ent, "Enter custom pattern match filter here. Note that \"string\" without * or ? achieves a match on \"*string*\".");
